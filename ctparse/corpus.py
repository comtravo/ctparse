...
def make_partial_rule_dataset(
    ...
    The text is run through ctparse and all parses (within the specified timeout and
    max_stack_depth) are obtained. Each parse contains a sequence of rules (see ``CTParse.rules``)
    used to produce that parse.

    A dataset is generated by taking every possible partial rule and assigining to it
